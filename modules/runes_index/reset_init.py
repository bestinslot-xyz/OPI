...
if init_env:
  ...
  res = input("Runes Postgres DB username (Default: postgres): ")
  if res != '':
    DB_USER = res
  res = input("Runes Postgres DB host (Default: localhost) leave default if postgres is installed on the same machine: ")
  if res != '':
    DB_HOST = res
  res = input("Runes Postgres DB port (Default: 5432): ")
  if res != '':
    DB_PORT = res
  ...
  if res != '':
    DB_SSL = res
  res = input("Runes Postgres DB max connections (Default: 50): ")
  if res != '':
  ...
  if res != '':
    ORD_DATADIR = res
  res = input("Network type (Default: signet) options: mainnet, testnet, signet, regtest: ")
  if res != '':
    NETWORK_TYPE = res
  res = input("Report to indexer (Default: true): ")
  if res != '':
...
res = input("Are you sure you want to initialise/reset the runes database? (y/n) ")
if res != 'y':
  print('aborting')
  exit(1)
...
db_exists = False
try:
  cur.execute('select count(*) from runes_block_hashes;')
...
ord_folder = os.getenv("ORD_FOLDER") or "../../ord-runes/target/release/"
...
ord_log_file_path = pathlib.Path(ord_folder, network_path, "runes_output.txt").absolute()
ord_log_file_path.write_text("")

ord_log_file_index_path = pathlib.Path(ord_folder, network_path, "runes_output_blocks.txt").absolute()
