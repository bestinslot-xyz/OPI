...
if init_env:
  ...
  use_other_env = False
  other_env_exists = os.path.isfile('../runes_index/.env')
  if other_env_exists:
    res = input(".env on runes_index already exists, do you want to use values from there? (y/n) ")
    if res == 'y':
      use_other_env = True
  if use_other_env:
    env = dotenv_values(dotenv_path='../runes_index/.env')
  ...
  if res != '':
    DB_SSL = res
  res = input("Runes Postgres DB max connections (Default: 10): ")
  if res != '':
    DB_MAX_CONNECTIONS = res
  res = input("API host (Default: 127.0.0.1): ")
  if res != '':
    API_HOST = res
  res = input("API port (Default: 8003): ")
  if res != '':
    API_PORT = res
