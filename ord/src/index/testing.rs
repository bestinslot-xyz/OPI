use {super::*, clap::Parser, std::ffi::OsString, tempfile::TempDir};
...
impl ContextBuilder {
  ...
  pub(crate) fn try_build(self) -> Result<Context> {
    let core = mockcore::builder().network(self.chain.into()).build();
    ...
    let options = crate::Options::try_parse_from(command.into_iter().chain(self.args)).unwrap();
